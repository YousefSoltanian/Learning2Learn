...
class ILQSolver(object):
  ...
  def run(self, x0, Ps_warmstart=None, alphas_warmstart=None):
    ...
    while (iteration <= self._max_iter):  #and (not self._is_converged_cost()):
      ...
      tt = time.time()
      As = np.asarray(jax.lax.stop_gradient(As))
      Bs_list = [np.asarray(jax.lax.stop_gradient(Bs)) for Bs in Bs_list]
      lxs = [np.asarray(jax.lax.stop_gradient(lxs_i)) for lxs_i in lxs]
      Hxxs = [np.asarray(jax.lax.stop_gradient(Hxxs_i)) for Hxxs_i in Hxxs]
      Huus = [np.asarray(jax.lax.stop_gradient(Huus_i)) for Huus_i in Huus]
